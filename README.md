...
```text
锁定身份与关系 → 找到一个情绪或方向命题 → 用稀少连续手绘线重组 → 分布二至四块源图衍生透明有机色域 → 让局部叠色形成克制综合色 → 保住纸白呼吸 → 让目标语言文字成为画中另一根线
```
...
完整审美约束与拒绝项写在 Skill 和生产提示词中；它们保留原始提示词的审美动机，但不会把历史 3:4 画布变成隐藏默认值。 [SKILL.md](SKILL.md) · [production prompt](references/xxd-panel-036-prompt.en.md)

## 样张 · 来自 X

> [小小东（@xiaoxiaodong01）](https://x.com/xiaoxiaodong01/status/2090745034257903827) · 2026-08-21<br>
...
<p align="center"><a href="https://x.com/xiaoxiaodong01/status/2090745034257903827">查看原推文与完整提示词 →</a></p>

这些样张用于展示 036 的美学动机，不会把样张中的主体、构图、配色、文案或旧画幅变成生成参考或当前默认值。
...
| 模式 | 画幅逻辑 | 成品 |
| --- | --- | --- |
| `top-bottom` | 用户确认的整张成品画幅 | 一次生成完整画布：高保真原图在上，036 设计在下，约 50/50 |
| `left-right` | 用户确认的整张成品画幅 | 一次生成完整画布：高保真原图在左，036 设计在右，约 50/50 |
| `design-only` | 用户确认的整张成品画幅 | 036 设计铺满画布，不显示原照片 |
...
本项目的文案规则： 只用一个与源图强绑定的极短词或短句。采用目标语言中自然的纤细手写或极轻编辑字体，让主体线成为起笔、收笔、基线、连接线或路径；最多增加一条微型辅助信息。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/036-source.md) · [英文生产提示词](references/xxd-panel-036-prompt.en.md) · [中文生产提示词](references/xxd-panel-036-prompt.zh-CN.md)
...
<div align="center"><strong>线保住辨识，颜色承接没有说出口的情绪。</strong></div>
